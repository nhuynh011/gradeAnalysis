...
# Currently set for cleaning MA232 (DiffEq) files only, read comments to see which one is needed for diffeq/calc3
...
# Format as numeric to help excel export (calc3):
#for (x in 2:18) {
#  df[x] <- as.numeric(unlist(df[x]))
#}
# Format as numeric to help excel export (diffeq):
for (x in 2:16) {
  df[x] <- as.numeric(unlist(df[x]))
}
